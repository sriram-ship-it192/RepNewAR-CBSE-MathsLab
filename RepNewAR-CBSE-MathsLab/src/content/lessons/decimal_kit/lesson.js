...
export class Lesson extends PhysicalMathLabLessonBase {
  constructor() {
    super({
      id: 'decimal_kit', title: 'Decimal Kit — Tenths & Hundredths', category: 'CBSE Maths Lab • Classes 3–5', difficulty: 'Beginner',
      description: 'Tags #2–#5 represent decimal place pieces; move them around the base marker.', kind: 'decimal', shortLabel: 'Decimal',
      activity: '0.37', supportedMarkerIds: [1,2,3,4,5],
      defaultValue: 37, min: 0, max: 100, step: 1
    });
  }
  ...
  _buildVisual() {
    ...
    g.add(this._text('Decimal Kit',.055));
    const ids=[...this._anchors.keys()].filter(id=>id>1);
    const live=this._anchors.has(1) && ids.length>0;
    const values={2:0.10,3:0.01,4:0.20,5:0.02};
    const total=ids.reduce((sum,id)=>sum+(values[id]||0),0);
    // AR place-value board: blue = tenths, green = hundredths.
    const board=new THREE.Mesh(new THREE.BoxGeometry(.52,.012,.40),this._material(0x10213d));
    board.position.set(0,-.028,0); g.add(board);
    for(let row=0;row<10;row++) for(let col=0;col<10;col++){
      const idx=row*10+col;
      const mat=this._material(0x24324a);
      const cell=new THREE.Mesh(new THREE.BoxGeometry(.043,.009,.031),mat);
      cell.position.set((col-4.5)*.047,.002,(row-4.5)*.034); g.add(cell);
    }
    const tenths=Math.round(total*10)%10;
    const hundredths=Math.round(total*100)%10;
    for(let i=0;i<tenths;i++){
      const m=new THREE.Mesh(new THREE.BoxGeometry(.043,.014,.031),this._material(0x38bdf8));
      m.position.set((i-4.5)*.047,.012,-.155); g.add(m);
    }
    for(let i=0;i<hundredths;i++){
      const m=new THREE.Mesh(new THREE.BoxGeometry(.043,.016,.031),this._material(0x22c55e));
      m.position.set((i-4.5)*.047,.016,.155); g.add(m);
    }
    g.add(this._text(live ? total.toFixed(2) : '0.00',.05));
    return g;
  }
  _angleVisual(g) {
