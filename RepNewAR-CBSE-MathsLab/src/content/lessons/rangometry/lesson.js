...
export class Lesson extends PhysicalMathLabLessonBase {
  constructor() {
    super({
      id: 'rangometry', title: 'Rangometry Kit — Angles', category: 'CBSE Maths Lab • Classes 3–5', difficulty: 'Beginner',
      description: 'Tag #1 is the pivot; Tag #2 is the moving rod. Rotate the physical rod to update the angle.', kind: 'angle', shortLabel: 'Angle',
      activity: '60°', supportedMarkerIds: [1,2,3,4,5],
      defaultValue: 60, min: 0, max: 180, step: 1
    });
  }
  _displayValue() { return `${Math.round(this._calculateMeasuredAngle())}°`; }
  _calculateMeasuredAngle() {
    const base=this._anchors?.get(1);
    const moving=this._anchors?.get(2);
    if(base && moving) {
      const world=new THREE.Vector3();
      moving.getWorldPosition(world);
      const local=base.worldToLocal(world);
      if(Math.hypot(local.x,local.z)>0.005) {
        let measured=THREE.MathUtils.radToDeg(Math.atan2(-local.z,local.x));
        measured=(measured+360)%360;
        return measured>180 ? 360-measured : measured;
      }
    }
    return this._demo;
  }
  ...
  _updateActivityState() {
    if(!this._ui) return;
    const st=this._ui.querySelector('#pml-status');
    const base=this._anchors?.has(1);
    const moving=this._anchors?.has(2);
    const a=this._calculateMeasuredAngle();
    const type=a===90?'Right angle':a<90?'Acute angle':a<180?'Obtuse angle':'Straight angle';
    if(st) st.innerHTML=(base && moving)
      ? `<span class=\"pml-ok\">● LIVE PHYSICAL TRACKING</span><br>Pivot: Tag #1 ✓<br>Moving rod: Tag #2 ✓<br>Measured: <b>${Math.round(a)}°</b> — ${type}`
      : `<span class=\"pml-warn\">● WAITING FOR PHYSICAL MARKERS</span><br>Pivot Tag #1: ${base?'✓ detected':'waiting'}<br>Moving rod Tag #2: ${moving?'✓ detected':'waiting'}<br>Demo angle: ${Math.round(this._demo)}°`;
    const v=this._ui.querySelector('#pml-value');
    if(v) v.textContent=`${Math.round(a)}°`;
  }
  ...
  _angleVisual(g) {
    // Tag #1 is the physical pivot/base. Tag #2 is the movable rod marker.
    // IMPORTANT: calculate the angle in Tag #1's LOCAL coordinate system so
    // the result is independent of how the printed base marker is rotated
    // relative to the webcam. This makes the AR rod follow the physical rod.
    let a=this._calculateMeasuredAngle();
    let endpoint=null;
    const base=this._anchors.get(1);
    const moving=this._anchors.get(2);
    if(base && moving) {
      const world=new THREE.Vector3();
      moving.getWorldPosition(world);
      endpoint=base.worldToLocal(world);
    }
    this._measuredAngle=a;

    const rad=THREE.MathUtils.degToRad(a);
    const mat=this._material(0x22c55e);
    const red=this._material(0xef4444);
    const blue=this._material(0x38bdf8);

    // Fixed 0° reference ray.
    const baseLen=.22;
    const baseRay=this._line(new THREE.Vector3(0,.008,0),new THREE.Vector3(baseLen,.008,0),0xef4444);
    g.add(baseRay);

    // Measured ray: if Tag #2 is visible, terminate at its actual local
    // position. Otherwise use the demo slider angle.
    let endX=.22*Math.cos(rad), endZ=-.22*Math.sin(rad);
    if(endpoint && Math.hypot(endpoint.x,endpoint.z)>0.005) {
      const len=Math.min(.38,Math.max(.08,Math.hypot(endpoint.x,endpoint.z)));
      const scale=len/Math.hypot(endpoint.x,endpoint.z);
      endX=endpoint.x*scale;
      endZ=endpoint.z*scale;
    }
    const rod=this._line(new THREE.Vector3(0,.014,0),new THREE.Vector3(endX,.014,endZ),0x22c55e);
    g.add(rod);

    // Highlight the tracked marker location.
    if(endpoint) {
      const marker=new THREE.Mesh(new THREE.SphereGeometry(.014,16,12),this._material(0xf59e0b));
      marker.position.set(endX,.018,endZ);
      g.add(marker);
    }

    // Measurement arc follows the same local coordinate frame.
    const r=.18;
    const pts=[];
    for(let i=0;i<=40;i++){const t=rad*i/40;pts.push(new THREE.Vector3(r*Math.cos(t),.018,-r*Math.sin(t)))}
    g.add(new THREE.Line(new THREE.BufferGeometry().setFromPoints(pts),new THREE.LineBasicMaterial({color:0x38bdf8})));

    const hub=new THREE.Mesh(new THREE.CylinderGeometry(.018,.018,.022,24),this._material(0xf59e0b));
    hub.rotation.x=Math.PI/2;
    g.add(hub);

    const type=a===90?'Right angle':a<90?'Acute angle':a<180?'Obtuse angle':'Straight angle';
    const label=this._text(`${Math.round(a)}° • ${type}`,.045);
    label.position.set(.04,.055,-.02);
    g.add(label);
  }
